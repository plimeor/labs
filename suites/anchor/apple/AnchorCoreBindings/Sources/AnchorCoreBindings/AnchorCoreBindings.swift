...
///
/// Forward-compatible: a code emitted by core that this mirror does not yet know
/// decodes to `.unknown(raw)` instead of failing the whole `TransactionResult`
/// decode, so vocabulary drift degrades gracefully rather than crashing the
/// binding. `rawValue` round-trips every case (including `.unknown`).
public enum ValidationErrorCode: Decodable, Equatable, Sendable {
    case invalidUTF16Offset
    case directActiveToDeleted
    case structuralDispatchDeferred
    case adapterNullSession
    case adapterParseError
    case unknown(String)

    public init(rawValue: String) {
        switch rawValue {
        case "invalid_utf16_offset": self = .invalidUTF16Offset
        case "direct_active_to_deleted": self = .directActiveToDeleted
        case "structural_dispatch_deferred": self = .structuralDispatchDeferred
        case "adapter_null_session": self = .adapterNullSession
        case "adapter_parse_error": self = .adapterParseError
    ...
    public var rawValue: String {
        switch self {
        case .invalidUTF16Offset: return "invalid_utf16_offset"
        case .directActiveToDeleted: return "direct_active_to_deleted"
        case .structuralDispatchDeferred: return "structural_dispatch_deferred"
        case .adapterNullSession: return "adapter_null_session"
        case .adapterParseError: return "adapter_parse_error"
...
private struct BlobID: Decodable {
    let blobID: String
...
public func blobID(bytes: Data) throws -> String {
    ...
    return try decode(BlobID.self, from: buffer).blobID
}
...
#else
public func openFixtureVault() throws -> FixtureSummary {
    throw AnchorBindingError.unavailableOnPlatform
}

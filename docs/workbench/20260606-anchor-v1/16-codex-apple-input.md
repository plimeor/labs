...
> 边界：本文件不创建 Apple 工程 / entitlements / container（Codex/用户授权范围）。core 已落在 `suites/anchor/core`（CP-0 Option A）。core 全树零云符号（见 `15-core-evidence.md` §4）。
...
# UniFFI Swift bindgen / 三 slice XCFramework / SwiftPM wrapper —— 见 12-stage-1-spike-plan.md §2
```
...
- **core 面 op-count 已证**：million-op replay = O(ops)，1.25M ops ≈ 2.6s release（`15-core-evidence.md` §6）；segment batching budget shape 证 N op 不产 ~N segment（F42，`segment.rs`）。

...
- **Status**：Recommended / delivery gate（B14 default transport approved 2026-06-07；signed macOS 10K/50K/100K direct enumeration observed；macOS package-internal placeholder/download returned `NSCocoaErrorDomain:4`；remote placeholder、iOS large-scale delivery、steady-state segment budget remain delivery gates）
- **Rationale**：iCloud Drive 无 delta 同步、任何改动整文件重传，不可变 segment 只上传一次、永不重传。
- **Evidence**：plan §8.4（不可变 segment 理由）、§11（同步单元 / segment 大小 / 提交节奏）。Codex §7.1：**Observed** — `ICloudAdapterProbe` 的 `NSMetadataQuery` / `NSFileCoordinator` adapter API 编译面 macOS + iOS sim 通过，core-only `SegmentId`/bytes 协议形态可保持；**Observed** — segment 写一次、新内容新 segment 形态可由 content hash + mtime 验证（设计层）。Stage 1 19-icloud-drive-report.md：**Observed** — signed app 在真实 account 写 1024 个 segment 文件（iOS ≈3720ms / macOS ≈3247ms），file-coordinated direct package-internal enumeration 见 1024 段，placeholder 下载行为 platform-sensitive（iOS `start_download=ok`、macOS `NSCocoaErrorDomain:4`）；signed macOS app 写 10K / 50K / 100K package-internal segment files，direct enumeration 分别见 10K / 50K / 100K，direct enumeration latency ≈27.70ms / 142.63ms / 299.51ms，package-internal metadata enumeration 仍为 0。Stage 1 33-icloud-placeholder-report.md：**Observed** — macOS loose iCloud file is ubiquitous/current, evict returns `NSCocoaErrorDomain:512`; package-internal segment is not ubiquitous and evict/start-download both return `NSCocoaErrorDomain:4`, while coordinated read still succeeds. **Not run** — true remote `.icloud` placeholder 下载、iOS large-scale delivery、million-op 后稳态 segment budget（见 D14/D35、12-stage-1-spike-plan.md §4）。
...
- **Evidence**：plan §8.4；conflict §10、§13.1 #6、§11。Phase 0 Codex §7.1/§7.4：**Observed** — `NSFileCoordinator` adapter API 编译面可用、无 entitlement CLI 下 `coordinated_bytes=7 coordinator_error=none`；付费 ADP team 已开通，entitlement / 真机 ubiquity lookup 前置满足。Stage 1 19-icloud-drive-report.md：**Observed** — shared-container macOS + iOS online concurrent manifest writes converged with `conflict_versions=0`; offline iOS / online macOS fork materialized 1 unresolved `NSFileVersion` conflict after reconnect; current file became `ios-offline`, retained conflict version contained `mac-online` JSON; no conflict resolution or deletion was performed. Stage 1 26-icloud-conflict-policy-floor-report.md：**Observed** — repo-local signed macOS verifier read the historical offline-conflict run, found current `manifest.json` writer `ios-offline`, unresolved conflict version writer `mac-online`, materialized duplicate `manifest 2.json` writer `ios-base`, and returned `adapter_status=blocked_manifest_conflict`, `policy_surface_conflicts=true`, `policy_preserve_versions=true`, `policy_auto_resolve=false`, `resolution_executed=false`. Stage 1 39-icloud-conflict-resolution-report.md：**Observed** — explicit `resolution_choice=current` archived current `ios-offline`, conflict version `mac-online`, and duplicate `ios-base`; moved duplicate materialization out of the live directory; `removeOtherVersionsOfItem(at:)` returned nil; same-run `after_conflict_versions=1` showed metadata lag; subsequent policy probe returned `conflict_versions=0`, `duplicate_manifest_files=0`, `adapter_status=ok_no_conflict`.
...
- **Evidence**：plan §8.2、§11；conflict §6.1（mark re-clamp 以 UTF-16 计）。Codex §1/§2.4/§6.1：**Observed** — macOS `NSTextView` runtime 可设 UTF-16 selection（`textkit:utf16=16 storage=16`、`textkit:selected=1:8`）。Stage 1 27-textkit-runtime-floor-report.md / 47-textkit-accessibility-range-report.md：**Observed** — Swift-side fixture counts for emoji / ZWJ / combining mark / CRLF / mixed text; IME marked-text commit maps to `EditorIntent.insertText(... atUTF16:1 ...)`; macOS `NSTextView.accessibilitySelectedTextRange()` reads back `1:2`; iOS Simulator `AnchorTextKitProbe` compile passed. **Not run** — core 内部存储单位与 Swift/Rust binding 边界的最终换算仍须冻结并验证；`UITextView` runtime IME/accessibility 行为与产品 accessibility mapping 未测。
...
- **Evidence**：conflict §6.1、§5.3、§10、§13.1 #4、§12；plan §8.2。Codex §1/§8.2：**Recommended（Observed-supported）** — diff3 / order-key 推荐只在 Rust core 执行，不由 Swift/TextKit 各自实现，跨平台一致由单一编译产物保证；fixture 的「双端逐字节一致」证据来自 Rust core vectors，不来自 TextKit/Swift 专属实现。Stage 1 `15-core-evidence.md` / `25-cross-target-runner-report.md` / `31-cross-target-ci-wiring-report.md` / `41-hosted-cross-target-ci-report.md` / `61-android-emulator-ci-wiring-report.md`：**Observed** — deterministic vectors include diff3, order-key, canonical rev, fixture snapshot, conflict snapshot, merged snapshot; native cargo test passed 6/0; wasm execution returns `anchor_wasm_vector_status=0`; iOS Simulator execution returns `anchor_ios_vector_status=0`; GitHub Actions workflow wiring added; hosted `pull_request` run `27225506636` passed Linux native+wasm and macOS native+wasm+iOS Simulator jobs; Android emulator workflow wiring exists and local no-NDK screening returns `anchor_android_vector_status=not_run_no_ndk`. Hosted Android execution remains open.
...
- **Evidence**：conflict §5.2、§6.4、§13.1 #5、§12；plan §8.2。Codex §1/§8.2：**Recommended（Observed-supported）** — order-key 与 diff3 同推荐只在 Rust core 执行，跨平台一致由单一编译产物保证（须核验无平台相关浮点/locale 介入）。Stage 1 `15-core-evidence.md` / `25-cross-target-runner-report.md` / `31-cross-target-ci-wiring-report.md` / `41-hosted-cross-target-ci-report.md` / `61-android-emulator-ci-wiring-report.md`：**Observed** — order-key vectors execute in native cargo test, wasm WebAssembly runtime, and iOS Simulator slice; GitHub Actions workflow wiring added; hosted `pull_request` run `27225506636` passed Linux native+wasm and macOS native+wasm+iOS Simulator jobs; Android emulator workflow wiring exists and local no-NDK screening returns `anchor_android_vector_status=not_run_no_ndk`. Hosted Android execution remains open.
...
- **Evidence**：Codex §2.6：**Observed** — `teamType = Individual`、`isFreeProvisioningTeam = 0`；demo project 含 iCloud Documents entitlement，device app 签名含匹配 iCloud entitlements；真机 iPhone 返回 non-nil ubiquity container URL。Stage 1 19-icloud-drive-report.md：**Observed** — physical iPhone signed runtime passed explicit/implicit container lookup, package type id, `NSFileCoordinator` read/write, current segment download call, and 1024-file write subset; real signed macOS app automatic signing/provisioning/build/runtime passed; macOS package-level metadata discovery passed; package-internal direct enumeration saw 1024 hidden `.anchor` segment files, 128 visible package-internal segment files, and 10K / 50K / 100K scale counts; package-internal `NSMetadataQuery` returned 0 at every tested scale; package-external metadata query found `.seg` files; online macOS+iOS concurrent writes converged with 0 conflicts; offline iOS / online macOS fork materialized 1 unresolved `NSFileVersion` conflict after reconnect; core cloud-symbol audit returned 0 matches. Stage 1 26-icloud-conflict-policy-floor-report.md：**Observed** — signed macOS verifier surfaced current `ios-offline`, unresolved conflict `mac-online`, duplicate `manifest 2.json` `ios-base`; policy floor returned `blocked_manifest_conflict`, preserve/surface true, auto-resolve false, resolution not executed. Stage 1 39-icloud-conflict-resolution-report.md：**Observed** — signed macOS verifier executed explicit current-winner resolution for that conflict, archived current/conflict/duplicate branches, moved the duplicate manifest out of the live directory, observed same-run conflict metadata lag, and then observed a subsequent policy read with `conflict_versions=0`, `duplicate_manifest_files=0`, `adapter_status=ok_no_conflict`. Stage 1 32-ios-device-icloud-launch-report.md / 37-ios-device-icloud-rerun-report.md / 42-ios-device-icloud-locked-rerun-report.md：**Observed** — current physical iPhone is available/paired; Xcode device build succeeded with CloudDocuments entitlements; `devicectl` install succeeded; runtime launch was not observed because SpringBoard denied launch while the device was locked across three launch attempts. Stage 1 45-ios-simulator-icloud-rerun-report.md：**Observed** — after the user stated the iOS Simulator is logged into iCloud, the Xcode-managed iOS 26.5 Simulator verifier build/install/launch succeeded with simulated CloudDocuments entitlement surface, but app-side explicit/implicit ubiquity lookup returned nil and `bird` logged `BRCloudDocsErrorDomain:153` / `iCloud Drive not supported`; this rejects current Simulator as a substitute for physical iOS runtime proof. Stage 1 33-icloud-placeholder-report.md：**Observed** — macOS loose iCloud file is ubiquitous/current but evict did not create an observable remote placeholder; package-internal segment is not ubiquitous and evict/start-download return `NSCocoaErrorDomain:4`, while coordinated read succeeds.
- **Risk**：iCloud Drive default transport carries unresolved delivery gates: true remote placeholder behavior, signed-out / over-quota states, physical iOS runtime after device unlock, iOS large-scale delivery behavior, local-only path-in-ubiquity edge cases, product resolver UX/core integration, and million-op core replay/compaction budget. Current iOS 26.5 Simulator CloudDocuments runtime is not a valid substitute for physical iOS runtime evidence. Package-internal discovery via `NSMetadataQuery` and package-internal availability via successful `startDownloadingUbiquitousItem` are not approved design assumptions.
...
- **Evidence**：plan §8.1（core 平台无关）、§3（UI 平台化、核心平台无关）；Codex §2.2：**Observed** — Rust host target `aarch64-apple-darwin` 已装。Stage 1 15-core-evidence.md §3/§4：**Observed** — `anchor-core`（`#![no_std]`+alloc、零外部依赖）`cargo build --target wasm32-unknown-unknown` 与 `--target aarch64-linux-android` 均 `Finished`；core 依赖政策以「零运行时依赖」满足；client 零真理逻辑 / core 零云符号 grep 红线 0 命中。Stage 1 `25-cross-target-runner-report.md` / `31-cross-target-ci-wiring-report.md` / `41-hosted-cross-target-ci-report.md` / `61-android-emulator-ci-wiring-report.md`：**Observed** — repo-local runner executes native + wasm + iOS Simulator vectors; `ANCHOR_SKIP_IOS=1` supports Linux native+wasm CI path; `.github/workflows/anchor-cross-target-vectors.yml` wires Linux native+wasm and macOS iOS Simulator jobs; hosted `pull_request` run `27225506636` passed both jobs; Android emulator workflow wiring now exists. Android execution remains open until hosted Android emulator returns `anchor_android_vector_status=0`; current local `ANDROID_HOME` path does not exist and no Android SDK / NDK / `adb` is available.

...
- **Evidence**：conflict §5.2、§6.4、§13.1 #5、§12；plan §8.2。Codex §1/§8.2：**Recommended（Observed-supported）** — order-key 与 diff3 同推荐只在 Rust core 执行，跨平台一致由单一编译产物保证（须核验无平台相关浮点/locale 介入）。Stage 1 `15-core-evidence.md` / `25-cross-target-runner-report.md` / `31-cross-target-ci-wiring-report.md` / `41-hosted-cross-target-ci-report.md` / `61-android-emulator-ci-wiring-report.md` / `62-hosted-android-cross-target-report.md`：**Observed** — order-key vectors execute in native cargo test, wasm WebAssembly runtime, iOS Simulator slice, and hosted Android emulator binary; GitHub Actions workflow wiring added; hosted `pull_request` run `27238297811` passed Linux native+wasm, macOS native+wasm+iOS Simulator, Ubuntu Android emulator, and Apple binding package jobs.
...
- **Evidence**：plan §8.1（core 平台无关）、§3（UI 平台化、核心平台无关）；Codex §2.2：**Observed** — Rust host target `aarch64-apple-darwin` 已装。Stage 1 15-core-evidence.md §3/§4：**Observed** — `anchor-core`（`#![no_std]`+alloc、零外部依赖）`cargo build --target wasm32-unknown-unknown` 与 `--target aarch64-linux-android` 均 `Finished`；core 依赖政策以「零运行时依赖」满足；client 零真理逻辑 / core 零云符号 grep 红线 0 命中。Stage 1 `25-cross-target-runner-report.md` / `31-cross-target-ci-wiring-report.md` / `41-hosted-cross-target-ci-report.md` / `61-android-emulator-ci-wiring-report.md` / `62-hosted-android-cross-target-report.md`：**Observed** — repo-local runner executes native + wasm + iOS Simulator vectors; `ANCHOR_SKIP_IOS=1` supports Linux native+wasm CI path; `.github/workflows/anchor-cross-target-vectors.yml` wires Linux native+wasm, macOS iOS Simulator, and Ubuntu Android emulator jobs; hosted `pull_request` run `27238297811` passed all cross-target jobs, including Android emulator with `anchor_android_vector_status=0`; current local `ANDROID_HOME` path does not exist and no local Android SDK / NDK / `adb` is available, but the hosted Android machine gate is closed.

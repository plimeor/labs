...
- **Evidence**：plan §8.1（core 平台无关）、§3（UI 平台化、核心平台无关）；Codex §2.2：**Observed** — Rust host target `aarch64-apple-darwin` 已装。Stage 1 15-core-evidence.md §3/§4：**Observed** — `anchor-core`（`#![no_std]`+alloc、零外部依赖）`cargo build --target wasm32-unknown-unknown` 与 `--target aarch64-linux-android` 均 `Finished`；core 依赖政策以「零运行时依赖」满足；client 零真理逻辑 / core 零云符号 grep 红线 0 命中。Stage 1 `25-cross-target-runner-report.md` / `31-cross-target-ci-wiring-report.md` / `41-hosted-cross-target-ci-report.md` / `61-android-emulator-ci-wiring-report.md` / `62-hosted-android-cross-target-report.md`：**Observed** — repo-local runner executes native + wasm + iOS Simulator vectors; `ANCHOR_SKIP_IOS=1` supports Linux native+wasm CI path; historical `.github/workflows/anchor-cross-target-vectors.yml` wired Linux native+wasm, macOS iOS Simulator, and Ubuntu Android emulator jobs; hosted `pull_request` run `27238297811` passed all cross-target jobs, including Android emulator with `anchor_android_vector_status=0`. Stage 1 `71-github-workflow-removal-report.md`：**Observed** — current head deletes `.github/workflows/anchor-cross-target-vectors.yml`; Android and wasm are not current-period support targets, and iOS/macOS build verification is local for the current period.

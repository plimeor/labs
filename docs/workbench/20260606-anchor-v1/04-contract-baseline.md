...
**Core 跨平台可编译性 = CP-1 受保护不变量（区分于延后的 client）。** 在 platform-agnostic Rust core 取向下，**`anchor-core`（含 `anchor-editor-core`）的 wasm32 + android 可编译性**不是延后项，而是 Apple-first 阶段的主动不变量：core 依赖须能编到非-Apple target，确定性 / merge 路径保持 `no_std`-friendly、不碰 OS 线程 / 文件系统 / 时钟 / 浮点；多目标编译（macOS slice + wasm32 + android）作为 CP-1 一道 gate，diff3 / order-key 一致性向量集纳入 wasm target。Stage 1 已有 native / wasm / iOS Simulator / Android emulator 执行 runner；历史上曾有 GitHub Actions workflow wiring，并观察到 hosted `pull_request` workflow 通过 Linux native+wasm、macOS native+wasm+iOS Simulator、Ubuntu Android emulator jobs，Android hosted job returned `anchor_android_vector_status=0` in run `<CI_RUN>`。按 2026-06-10 当前期范围，`.github/workflows/anchor-cross-target-vectors.yml` 已暂时移除：Android 和 wasm 不作为本期支持目标的 hosted gate，iOS/macOS 构建验证走本地路径。受保护的是「core 在 Apple-first 阶段保持可复用」，**web / android 的 client 仍延后**到 macOS + iOS 跑通之后 [05-key-decisions.md D36、D37、D19、D26]。
...
- **单一已校验 dispatch**：首期要建立并守住的不变量——每条持久写入路径都调用「append 前校验」的 core 私有 helper，对 core 写入点 grep 必须能证明 [plan §8.5]。**Stage 1 / CP-2 Observed（23-cp-2-core-readiness.md）：** 本地 dispatch 收敛到唯一 appender `Session::commit`（先过 `validate_batch` 的 D10/D20 终态守卫），sync ingestion 是独立 op-log owner `IngestState::ingest`（dedup + per-actor HWM 守卫）；`rg "self\.log\.(extend|push)" suites/anchor/core/src` 恰 2 命中，各在其 struct 的已校验 chokepoint 内；被拒 dispatch 不产生部分写入（`dispatch_invariant.rs`）。
...
- **Toolchain 可用 = Observed（apple-verification §2.2、§4.1）。** 默认 `xcode-select` 指向 `/Library/Developer/CommandLineTools`，该默认态下 `xcodebuild` 失败；但用一次性命令前缀 `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer` 观察到 Xcode 26.5（Build 17F42）、macOS SDK 26.5、iOS SDK 26.5、iOS Simulator SDK 26.5，以及 iOS 26.2 / 26.4 / 26.5 模拟器设备；一台 iOS 26.5 模拟器可 bootstatus 到 Finished 并 shutdown。SwiftPM（Swift 6.3.2）可用。
...
- **Stage 1 spike build = Observed（17-apple-binding-evidence.md / 18-textkit-adapter-evidence.md / 19-icloud-drive-evidence.md）。** `anchor-core` builds for macOS / iOS / iOS-sim; C ABI and UniFFI wrappers create three-slice XCFrameworks; SwiftPM wrapper imports; TextKit and iCloud adapter compile surfaces pass; Swift wrapper actor async surface passes macOS Release strict concurrency runtime and iOS Simulator Release strict compile; UniFFI generated async surface passes macOS runtime, iOS Simulator compile/link, and iPhoneOS standalone `arm64` compile/link; repo-external SwiftPM path consumer imports `AnchorCoreBindings` and runs the async surface; repo-external binary-target consumer imports raw C ABI XCFramework and runs fixture lookup; repo-external wrapper binary package consumes `AnchorCoreBindings` over a binary XCFramework and runs fixture/dispatch/error/segment truth; hosted macOS runner builds the C ABI wrapper binary package, computes SwiftPM checksum, and reruns UniFFI generated async macOS runtime + iOS Simulator/iPhoneOS compile-link; core-owned `TransactionResult` / `ValidationError` vocabulary is frozen as a structured envelope plus exactly three core validation codes. Repo-local product Anchor app target / scheme remains outside this spike and is **Not run**.
- **Rust Apple targets = Observed（17-apple-binding-evidence.md）。** `aarch64-apple-darwin`、`aarch64-apple-ios`、`aarch64-apple-ios-sim` are installed on this machine. CI/dev machines require explicit target checks.
...
- **Recommended path = UniFFI（structured DTO / error / normal dispatch）+ generated Swift + Rust static libraries 打成 XCFramework + 由 local SwiftPM wrapper / binary target 消费；bulk segment/blob bytes 使用 C ABI fast path**（17-apple-binding-evidence.md）。
- **可行性证据 = Observed（apple-verification §2.4、§5.1、§5.3；17-apple-binding-evidence.md；17-apple-binding-evidence.md）：** C ABI + Rust staticlib + SwiftPM host path 实际跑通；UniFFI 0.31.1 经项目内 `cargo run -p uniffi-bindgen-swift` 生成并运行 minimal record/bytes Swift binding；Stage 1 完成 Anchor DTO / fixture / `TransactionResultSummary` / typed `ValidationError` / segment bytes / blob bytes full round-trip，C ABI 与 UniFFI 三 slice XCFramework 均创建成功，generated Swift synchronous smoke 通过 Swift 6 strict concurrency + warnings-as-errors；SwiftPM wrapper 层 `AnchorCoreClient` actor async surface 通过 macOS Release strict concurrency runtime 与 iOS Simulator Release strict compile。
- **C ABI bytes fast path = Recommended（17-apple-binding-evidence.md）。** C ABI 64MB bytes path 约 38.22ms / max RSS 约 96MB；UniFFI 64MB 约 145.22ms / max RSS 约 267MB。DTO / ordinary dispatch 走 UniFFI，bulk bytes 走 C ABI。
- **Caveat（必须随 binding 决策携带）：** Swift wrapper actor async surface 已证可行；core-owned final DTO/error vocabulary 已冻结；complete wrapper binary package mechanism floor 已证可行；UniFFI generated async surface 已过 macOS runtime + iOS Simulator compile/link + iPhoneOS standalone `arm64` compile/link 机制下限；SwiftPM binary checksum mechanism 已证可行；hosted/fresh-runner binding package reproduction 已对 verifier artifacts 关闭；artifact provenance/signing/notarization policy floor 已冻结为「verifier checksum/provenance 与 product app signing/notarization 分层」；先前 iPhoneOS `arm64e` standalone link failure 是 target 与 Rust `aarch64-apple-ios` slice 不匹配，不再作为 `arm64` iPhoneOS 机制 blocker；signed app bundle、physical-device install/runtime、actual Developer ID notarization/App Store distribution 与 real release upload/distribution channel 仍未关闭。
...
- **Binding 机制作为产品分发边界冻结 = Approved（B4，2026-06-07）。** 批准形状为 `UniFFI DTO / ordinary dispatch + C ABI bytes fast path`；Swift wrapper actor async surface、UniFFI generated async mechanism floor（含 iPhoneOS standalone `arm64` compile/link 和 physical-device verifier runtime）、repo-external SwiftPM path consumer、raw C ABI binary-target import、complete wrapper binary package、SwiftPM checksum mechanism、hosted/fresh-runner package reproduction、artifact provenance policy floor、development-signed verifier app-bundle/device runtime、core-owned final DTO/error vocabulary 已有 Stage 1 机制证据；remaining release implementation gates 为 product app binding integration、actual Developer ID notarization/App Store distribution 与 real release upload/distribution channel。
...
- **iCloud Drive vault file package 必须声明符合 `com.apple.package` 的 exported document type / UTType（Observed-supported，apple-verification §7.1；19-icloud-drive-evidence.md）。** Vault/package discovery 使用 package-level metadata；package-internal segment discovery 使用 file-coordinated direct enumeration，不能依赖 `NSMetadataQuery` 枚举 package internals。
...
- **iCloud Documents / ubiquity runtime = approved default transport with compromise constraints（apple-verification §2.6、§7；19-icloud-drive-evidence.md；19-icloud-drive-evidence.md；19-icloud-drive-evidence.md；19-icloud-drive-evidence.md；19-icloud-drive-evidence.md；19-icloud-drive-evidence.md；19-icloud-drive-evidence.md；19-icloud-drive-evidence.md；19-icloud-drive-evidence.md；19-icloud-drive-evidence.md；19-icloud-drive-evidence.md；user approval 2026-06-07）。** 用户已开通付费 Apple Developer Program（Individual）team（`isFreeProvisioningTeam=0`）；signed iPhone and macOS probes returned non-nil ubiquity containers, validated package type id, coordinated read/write, 1024-file write subset, macOS 10K/50K/100K package-internal direct enumeration, default 1M-op steady-state 124-file iCloud package-internal budget floor, package-level metadata discovery, package-internal metadata miss, and offline `NSFileVersion` conflict materialization. Mutable manifest conflict surfaces include unresolved `NSFileVersion` conflicts and duplicate `manifest *.json` materialization; the minimum adapter policy is surface current/conflict/duplicate branches, preserve them, block manifest-derived frontier/GC/cursor decisions, and never auto-resolve. Explicit current-winner resolution has been observed preserving current/conflict/duplicate branches into an archive, moving the duplicate manifest out of the live directory, and reaching `adapter_status=ok_no_conflict` on a subsequent policy read; same-run `NSFileVersion` metadata may lag destructive calls, so adapters must re-check before advancing. 2026-06-10 physical iPhone runtime returned explicit/implicit ubiquity container URLs, observed the vault package as ubiquitous, and passed file-coordinated segment write/read; the same run still reported `segment_is_ubiquitous=false`, `start_download_error=NSCocoaErrorDomain:4`, metadata gather `false`, and metadata segment count `0`, so it proves local device container/file coordination, not remote package-internal delivery. 2026-06-10 iOS 26.5 Simulator rerun built, installed, and launched the Xcode-managed verifier with simulated CloudDocuments entitlements, but app-side lookup returned explicit/implicit nil and `bird` logged `BRCloudDocsErrorDomain:153` / `iCloud Drive not supported`; this does not negate the user-provided login precondition, but it means current Simulator runtime cannot close iOS delivery. macOS placeholder probe observed loose iCloud file as ubiquitous/current while package-internal segment remained non-ubiquitous and returned `NSCocoaErrorDomain:4` for evict/start-download. macOS local-only path probe observed normal sandbox local vault allowed with backup exclusion, direct iCloud vault blocked, local symlink resolving into iCloud blocked, and iCloud symlink resolving to local blocked by raw-path policy. Remote `.icloud` placeholder, signed-out / over-quota, iOS remote/metadata delivery, remaining local-only path edge cases, product resolver UX/core integration, product compaction integration, and million-op iCloud product context remain delivery gates.
- **iCloud account-state classifier floor = Observed（19-icloud-drive-evidence.md）。** `AnchorICloudDriveProbe` classifies explicit nil + implicit nil ubiquity container URLs as `blocked_no_ubiquity_container`, and `AnchorAppleSmoke` observes `icloud:account_state_classifier=blocked_no_ubiquity_container explicit=false implicit=false` under SwiftPM runtime, Xcode Release strict build, Release executable run, and iOS Simulator compile surface. This closes only the no-container classifier mechanism floor; true signed-out runtime, over-quota runtime/write-failure behavior, and product UI recovery remain delivery gates.
- **iCloud steady-state segment budget file-count floor = Observed（19-icloud-drive-evidence.md）。** Core `segment_budget` tests pass, release `scale_bench` passes in the current local run, and the signed macOS verifier writes / direct-enumerates the default 1M-op steady-state file budget of 124 package-internal `.anchorseg` files with explicit and implicit ubiquity containers non-nil. Package-internal `NSMetadataQuery` remains 0 for `.anchorseg`, preserving the direct-enumeration adapter constraint. This closes only the budget-to-iCloud-file-count mechanism floor; product compaction integration, million-op iCloud product context, remote placeholder, account states, iOS remote/metadata delivery, and product resolver integration remain delivery gates.
...
- **local-only 语义（硬规则，用户 2026-06-07 firming）**：**`sync = "none"` 的 vault 严禁置于任何 iCloud ubiquity container 下**；放非 ubiquity 目录、标 `NSURLIsExcludedFromBackupKey` 排除备份、core 不挂任何 adapter、唯一出网点 `OpSyncPort::push` 受 adapter 门控可 grep 审计；**vault-open 强制断言路径不在任何 ubiquity container 下，否则拒开报警**；**`synced → local-only` 不可逆**（已离开设备的字节无法收回）[plan §8.4；05-key-decisions.md D21]。Stage 1 path classifier floor：raw path、resolved symlink target、raw `isUbiquitousItem`、resolved `isUbiquitousItem` 任一命中即 blocked；允许的 local vault 须成功设置并读回 backup exclusion（19-icloud-drive-evidence.md）。
- **local-only 误放入 iCloud = blocked open（D21a）。** 用户绕过 Anchor 手动把 `sync = "none"` vault 挪进 ubiquity container 后，Anchor **不自动转同步**，而识别为 **blocked misplacement**：拒开、不挂 `OpSyncPort` adapter、不上传 / 拉取 / merge，返回 typed `local_only_vault_in_ubiquity`（CLI exit 4、payload 含 `recommendedActions = ["move_back","convert_to_icloud_sync"]`），提供 **Move Back** / **Convert to iCloud Sync** 两动作。**隐私边界**：OS 可能已上传，故只声称「Anchor 不从 iCloud 管理位置打开 / 续写 local-only vault」，**不**声称「字节绝不离开设备」。Direct iCloud path、local symlink→iCloud、iCloud symlink→local 均按 blocked open 处理；external volume / security-scoped bookmark / `.icloud` placeholder / signed-out 仍未关闭 [05-key-decisions.md D21a；19-icloud-drive-evidence.md]。
...
> **加密信封边界（D22，已锁定 Phase-0 范围）：** 首期不实现 zero-knowledge；阶段0 锁定 `OpSyncPort` 与持久化之间的 **encryption envelope 缝**（首期 no-op / pass-through，日后接客户端加密层不迁移 op-log）+ **non-ZK 文案**，**绝不对中立 object-store 暗示 zero-knowledge**；完整 ZK / 密钥分发延后。**Mirror 目录组织（D05，已批准 2026-06-07）：** 人类可读路径 + 不纳入版本库（默认锁定）。**segment 大小与提交节奏、compaction GC 保留窗口与 manifest / cursor 协调**仍为 Phase-0 / Stage 1 收口项；其中**不可变 segment 文件**与**任何可变 manifest** 的区分须明确。Stage 1 证明 offline fork 可 materialize `NSFileVersion` unresolved conflict；product conflict policy floor 是 surface / preserve / block / no-auto-resolve；explicit current-winner resolution 机制已证实可先归档 current/conflict/duplicate branches 再清理 live conflict surface，但 product resolver UX/core integration 与 scale context 仍是 gate。**Sync 路线作为首期产品选择（B14）已由用户批准（2026-06-07）；local-only「sync=none 严禁置 iCloud」已由 D21 firm。** segment 大小 / batching policy / 长期 segment-file budget 按 op-count 与 synced-segment-file-count 两轴分离：core budget 排除每 op 一 segment，signed macOS verifier 已观察默认 1M-op steady-state 124-file budget 可在 iCloud package 内写入 / direct-enumerate；product compaction integration 与 million-op iCloud product context 仍为 delivery gates。manifest 默认取 per-device immutable cursor；**iCloud Drive 作首期 default transport 的交付 gated on delivery gates**（remote placeholder、account states、iOS remote/metadata delivery、product compaction integration、product resolver integration at scale；no-go-like delivery failure triggers design reset, not silent CloudKit migration）。iOS 26.5 Simulator 复测在登录前提下仍返回 `BRCloudDocsErrorDomain:153` / `iCloud Drive not supported`，不能替代 physical iOS runtime。physical iPhone 证据已关闭本机 container/file coordination floor，但未关闭 remote placeholder 或 package metadata delivery。macOS package-internal placeholder/download 当前证据为 `NSCocoaErrorDomain:4`，adapter 必须保留 direct enumeration/read 路径，不得依赖 package-internal `startDownloadingUbiquitousItem` 成功。详 12-stage-1-spike-plan.md §4/§6、19-icloud-drive-evidence.md。
...
**TextKit = mechanism-only（Observed，apple-verification §6；18-textkit-adapter-evidence.md）。** Apple 文档把 TextKit 定位为 text storage/layout infrastructure，匹配 Anchor 对其 input / layout / selection / hit-testing / IME / accessibility range surface 的角色。Codex 已证：`NSTextView` / `UITextView` adapter 编译面在 macOS + iOS simulator 通过；macOS `NSTextView` runtime 可设 UTF-16 selection、layout manager / text container 可用、`UndoManager` semantic undo closure 可执行（`textkit:undo_events=semantic-inverse-intent`）；controlled macOS probe 可观察 IME marked-text commit 到 `EditorIntent.insertText`、hit-testing insertion index、`allowsUndo=false` 下直接 buffer mutation 不被 `textView.undoManager?.undo()` 回滚、`accessibilitySelectedTextRange()` 回读当前 UTF-16 selected range（`textkit:accessibility_selected_range=1:2`）、真实 AppKit `NSView` / `NSTextView` hierarchy 对 text surface insert / move / remove patch 的 view lifecycle replay（`textkit:appkit_view_lifecycle=insert_move_remove`）、AppKit accessibility children 对多个 text surface 的 label / selected range 表达（`textkit:appkit_accessibility_children=2 ranges=1:2,0:3`）、AppKit selector / menu item sender 对 split / merge-backward intent 的路由（`textkit:appkit_menu_commands=blk_a:split@1,code_1:merge_backward`）、AppKit responder-chain `undo:` selector 到 adapter-owned semantic undo 的路由且 buffer 不变（`textkit:appkit_responder_undo=semantic-inverse-intent buffer_unchanged=true`）、AppKit split/scroll hierarchy 内 first-responder focus lifecycle 与 selection 保持（`textkit:appkit_focus_lifecycle=split_scroll blk_a->code_1 selections=1:0,0:0`）、iOS Simulator `UITextView` runtime 对 UTF-16 selection、marked-text range/commit、`UIKeyInput` insert/delete intent capture、scroll-hosted text surface identity / accessibility label 的表达（`textkit:ui_textview_runtime=selection=1:2 marked=1:1 commit=A拼 input=split@1,merge_backward hierarchy=blk_a,code_1 labels=2`）、iOS Simulator `UIScrollView` / `UITextView` hierarchy 对 insert / move / remove patch 的 view lifecycle replay（`textkit:ui_view_lifecycle=insert_move_remove`）、iOS Simulator `UIKeyCommand` + `UITextView` responder target-action 对 split / merge-backward command 的路由（`textkit:ui_menu_commands=blk_a:split@1,code_1:merge_backward`），以及 iOS Simulator `UITextView` responder target-action 到 adapter-owned grouped `UndoManager` semantic inverse 的路由且 buffer 不变（`textkit:ui_responder_undo=semantic-inverse-intent buffer_unchanged=true`）。**TextKit / `NSTextView`-`UITextView` / `NSRange`-`NSTextRange` / 平台 selection / view identity / 滚动-焦点-IME state / accessibility range / menu command sender / AppKit undo selector / UIKit command target-action / UIKit adapter-owned undo manager / split-scroll focus state 只作输入、排版、显示、命中测试与可访问性映射机制，永不作持久化或文档模型**；`anchor-editor-core` 同样不拥有它们 [plan §8.1、§4.2、§13]。Undo 接 `NSUndoManager`，但 undo 动作 dispatch inverse intent / op，不直接改 TextKit buffer [plan §8.1]；若 undo 能在不产生 core op 的情况下改 TextKit buffer，则 editor boundary 失效（apple-verification §6.4）。

**TextKit runtime status = partial observed（18-textkit-adapter-evidence.md）。** Stage 1 covers macOS UTF-16 emoji/ZWJ/combining/CRLF offset smoke, layout, selection, semantic undo through adapter-owned `UndoManager`, controlled IME marked-text commit mapping, TextKit hit-testing insertion index, controlled direct-buffer undo suppression, macOS accessibility selected range readback, adapter view-model projection patch replay for insert/move/remove text surfaces, macOS AppKit `NSEvent.keyDown` capture for Enter/Delete into split/merge-backward intents, real AppKit `NSView` / `NSTextView` hierarchy insert/move/remove lifecycle replay, macOS AppKit first-responder keyboard routing across two text surfaces, macOS AppKit accessibility hierarchy labels + selected ranges across two text surfaces, macOS AppKit selector / menu item sender routing for split / merge-backward intents, macOS AppKit responder-chain `undo:` selector routing to semantic undo with TextKit buffer unchanged, macOS AppKit split/scroll focus lifecycle across two text surfaces, iOS Simulator compile, actual iOS Simulator `UITextView` runtime for UTF-16 selection / marked-text commit / `UIKeyInput` split-merge capture / scroll-hosted identity + accessibility labels, iOS Simulator `UIScrollView` / `UITextView` lifecycle replay for insert/move/remove text surfaces, iOS Simulator `UIKeyCommand` target-action routing to split / merge-backward intents, and iOS Simulator `UITextView` responder target-action undo routing to adapter-owned grouped semantic undo with text buffer unchanged. Product AppKit view hierarchy integration, product app-hosted UIKit menu command system, full product focus lifecycle across document windows / tabs / sheets / sidebars / restored scroll state, product accessibility mapping, VoiceOver/UI runtime, product undo grouping / inverse-op dispatch, polished cross-block continuous native text selection, app-hosted UIKit focus/window lifecycle/accessibility behavior, product app-hosted UIKit patch replay, and real `anchor-core::dispatch` integration remain runtime gates.

**TextKit dispatch bridge status = partial observed（18-textkit-adapter-evidence.md；23-cp-2-core-readiness.md）。** Probe-level adapter insert intent reaches real `AnchorSession.dispatchInsertText` through `AnchorAppleSmoke` and returns `textkit:core_dispatch_bridge=insert changed=blk_a selection=2:2 segment=979` under SwiftPM runtime and Xcode Release strict build. Probe-level structural split / merge-backward intents first reached real `anchor-core` dispatch as core-owned typed deferral in doc 66; doc 72 advances the normal path to successful core dispatch and Swift bridge smoke output `textkit:core_dispatch_bridge=structural split=changed:blk_a,blk_op_split_block_10003 merge=changed:blk_a,blk_b`; doc 73 adds core-sourced `TransactionResult.editor_patches` and observes Swift/TextKit projection application from core patches (`patches=split:2,merge:2`); doc 74 adds core-owned `TransactionResult.undo_group` and observes Swift/TextKit inverse projection application (`undo=split:2,merge:2`); doc 75 adds core-only `Session::dispatch_undo_group` replay and observes insert/split/merge undo groups lowering to committed core ops in Rust tests. The remaining `anchor-core::dispatch` integration gate is product app-hosted dispatch for keyboard/menu/focus flows, Swift/FFI undo-group input, product `NSUndoManager` grouping/redo, mark-preserving full inverse-op contract beyond the current patch lower bound, split/merge concurrent intent rebase（auto-resolve；`split_merge_structural` conflict materialization surfacing + macro 原子消费 + op-shape 冻结已于 CP-2 落地，见 `23-cp-2-core-readiness.md`），and product AppKit/UIKit view hierarchy integration.
...
**UTF-16 边界（apple-verification §8.1 D18；18-textkit-adapter-evidence.md）。** 行内 offset 对外以 UTF-16 code unit 表达（对齐 Apple TextKit / Swift String bridging）为合理 Apple 边界；Stage 1 已观察 Swift-side emoji / ZWJ / combining mark / CRLF / mixed fixture 计数与 IME marked-text commit offset。core 内部存储单位与 Swift/Rust binding 边界的最终换算仍须冻结并验证。

**diff3 + fractional order-key 只在 Rust core 执行（Recommended，apple-verification §1、§8.4）。** 不由 Swift / TextKit 各自实现；同一编译产物经 binding 调用 → 跨 macOS / iOS 逐字节一致 by construction。fixture 的「双端逐字节一致」证据来自 Rust core consistency vectors，**不**来自 TextKit/Swift 专属实现；native / wasm / iOS Simulator / Android emulator execution runner 曾通过 hosted Linux native+wasm、macOS native+wasm+iOS Simulator、Ubuntu Android emulator jobs（见 `06-fixture-set.md` F23 / F26；`20-cross-target-ci-evidence.md`；`20-cross-target-ci-evidence.md`；`20-cross-target-ci-evidence.md`；`20-cross-target-ci-evidence.md`）。当前 head 暂时移除 GitHub Actions workflow；历史证据保留，但本期不再用 hosted Android/wasm workflow 作为持续 gate。

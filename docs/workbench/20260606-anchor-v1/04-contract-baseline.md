...
**TextKit dispatch bridge status = partial observed（63-textkit-core-dispatch-bridge-report.md）。** Probe-level adapter insert intent reaches real `AnchorSession.dispatchInsertText` through `AnchorAppleSmoke` and returns `textkit:core_dispatch_bridge=insert changed=blk_a selection=2:2 segment=979` under SwiftPM runtime and Xcode Release strict build. The remaining `anchor-core::dispatch` integration gate is product app-hosted dispatch for keyboard/menu/undo/focus flows, inverse-op undo grouping, non-insert editor operations, and core-sourced `EditorPatch` application.

...
**Where we are right now (cursor).** CP-0 is **approved** (2026-06-07). The **Claude / core half of CP-1 is complete** (74 tests pass, multi-target compile + zero-cloud-symbol gates green). The **Codex / Apple half of CP-1** has **approved boundaries** — B4 binding boundary and B14 iCloud default transport are both **user-approved 2026-06-07** — but still-open **implementation / delivery gates**. CP-1 has **not** exited as a whole. See §6 for the exact next action.
...
**PROJECT IS AT: CP-1, Apple half, mid-flight.**

**Settled (do NOT re-open or re-prove as new progress):** core half complete (74 tests; 1 ignored scale bench), multi-target compile (wasm32 + aarch64-linux-android) green, zero-cloud-symbol `rg` audit (0 matches, exit 1), golden `snapshot_revision 3ef88671…` / journal id `jnl_f99080…` / BLAKE3 vectors frozen, **B4 binding boundary approved 2026-06-07**, **B14 iCloud default transport approved 2026-06-07** (commits `096c4d6`, `1fb1934`), Option A layout `suites/anchor/{core,cli,apple,fixtures}` approved.

**THE SINGLE NEXT CONCRETE ACTION: dispatch the next Codex/Apple verifier round** to close the still-open CP-1 implementation/delivery gates enumerated in `21 §6` and `12 §6`. Run with the exact commands / env / stop conditions in `21 §9`:
...
**Numbering.** Strict `NN-semantic-name.md`, monotonically increasing, **never reused**. A new evidence doc takes the **next free `NN`** and a **kebab** semantic name (e.g. `22-cp-1-exit-report.md`). Current sequence: `01` plan, `02` conflict, `03` research, `04` contract-baseline, `05` key-decisions, `06` fixture-set, `07` layout-options, `08` codex-verification-packet, `09` apple-verification, `10` cp-0-approval, `11` cp-0-final, `12` stage-1-spike-plan, `13` stage-1-entry-brief, `14` core-spike-report, `15` core-evidence, `16` codex-apple-input, `17` apple-binding-report, `18` textkit-adapter-report, `19` icloud-drive-report, `20` apple-integration-state-for-claude, `21` stage-1-integration-report. (This driver is `00`.)

...
日期：2026-06-07
状态：**workbench artifact** —— 非公开接口契约。本文件是 CP-0 的最终索引：一段最强结论 + packet 文件索引 + CP-0 已冻结结论 + Stage 1 后当前状态 + 后续 gate 中明确禁止的顺手扩展 + 下一步执行入口。结构化批准摘要（含九项最终批准字段）见同目录 `10-cp-0-approval.md` §0，逐条勾选证据见其 §8。
...
>
> 引用约定：`[plan §X]` 指 `01-apple-native-note-workbench.md`；`[conflict §X]` 指 `02-conflict-resolution-model.md`；同目录姐妹文件以相对名引用。
...
| 文件 | 角色 |
|---|---|
| [plan] `01-apple-native-note-workbench.md` | 产品 / 架构方向摘要（历史记录，非当前接口契约）；CP-0 指针在顶部 + §11 阶段0 / CP-0 |
| [conflict] `02-conflict-resolution-model.md` | 冲突处置模型 v1（§8.3–8.5 增强）；三 register 之上的确定、可 replay、无静默丢失合并模型 |
| `10-cp-0-approval.md` | **最终用户批准稿**：§0 九项结构化批准摘要 + §8 逐条勾选清单 + §9 停止条件 |
| `04-contract-baseline.md` | CP-0 责任基线 + 五责任面 Responsibility boundary matrix（谁拥有 / 禁止拥有什么） |
| `05-key-decisions.md` | 整合决策表 D01–D38（含 D18a / D21a / D38），每项 Decision / Status / Rationale / Evidence / Risk / Stop condition |
| `06-fixture-set.md` | CP-0 设计 fixture set F01–F43（含 F42 segment budget、F43 四-horizon retention） |
| `07-project-layout-options.md` | 候选 A/B/C 对比；Primary = Option A `suites/anchor/*`，Fallback = Option C |
| `09-apple-verification.md` | Codex Step-2 Apple/Xcode/Swift-Rust/TextKit/iCloud 现实性核验证据（Observed / Inferred / Recommended / Unknown / Not run） |
| `12-stage-1-spike-plan.md` | Stage 1 五组 spike 计划（owner / commands / expected artifact / evidence / failure condition）+ CP-1 退出条件 |
| `13-stage-1-entry-brief.md` | 下一轮 agent 的 CP-1 启动 briefing（objective / scope / non-goals / owner work items / required reads / commands / CP-1 evidence / pause conditions） |
| `./17-apple-binding-evidence.md` / `18-textkit-adapter-evidence.md` / `19-icloud-drive-evidence.md` | Stage 1 Codex/Apple 实测报告：binding、TextKit、iCloud Drive runtime |
| `./15-core-evidence.md` / `14-core-spike-report.md` | Stage 1 Claude/core owner 实测报告：deterministic core、multi-target gate、mirror/search parity |
| `03-research-notes.md`、`08-codex-verification-packet.md` | 早期调研与 Codex 验证原始记录（背景，非批准面） |

## 3. CP-0 frozen conclusions（已冻结结论）

冻结 = 进入 CP-1 的固定前提，实现不得在未触发暂停条件的情况下偏离。完整论证见各责任文件，权威批准状态见 `10-cp-0-approval.md` §0 / §8。

- **产品形态与平台路线：** Note 原生、Apple 原生优先；macOS + iOS 首期 → iPadOS 第二 → 其他平台最后（A1；plan §3、§11）。
- **责任边界：** `anchor-core` 拥有真理 / 模型 / 校验 / 规范化 / op-log / replay / 全部 merge 规则 / diff3 + order-key 唯一执行处 / 单一已校验 dispatch / `OpSyncPort` trait / DTO + schema envelope；`anchor-editor-core` 是 core 内部无 UI 模块（非独立 crate/package），只拥有 selection / intent shaping / patch 映射；Apple 客户端 / CLI / `OpSyncPort` 适配器都是 dispatch 外壳、不拥有业务真理（第 2 节；04-contract-baseline.md）。
...
- **Apple binding 方向（A2 / B4 / D01）：** 用户已于 2026-06-07 批准 **UniFFI DTO / ordinary dispatch + C ABI bytes fast path** 作为 Apple binding 产品边界。Observed surface includes typed `ValidationError`, full UniFFI round-trip, C ABI/UniFFI three-slice XCFrameworks, synchronous Swift 6 strict-concurrency release smoke, and 1/4/16/64MB bytes benchmarks. Release implementation gates are UniFFI async-`Sendable` review, final DTO/error vocabulary, product wrapper import surface, and CI/fresh-machine reproduction.
...
当前入口是 `./21-stage-1-integration-report.md`。下一步只处理剩余 open gates：binding release implementation gates、iCloud transport delivery gates、TextKit product-runtime gate、Bun glob check、cross-target execution CI wiring。通过 CP-1 前不实现持久应用写入。

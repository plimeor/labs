...
| full `anchor-editor-core` intent surface | **partial（round 2 扩展）** | +`DeleteText`/`ReplaceText`/`RemoveMark`/`CreateBlock`；嵌入编辑器/富 inline/多块选区 remaining，见 `25` |
...
| full editor-core intent surface | partial（round 2 扩展，见 `25`） | Claude/core |

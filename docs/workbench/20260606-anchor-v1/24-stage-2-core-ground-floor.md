...
| op-segment **codec**（round-trippable serialize↔deserialize） | **done（round 2）** | `codec::encode_segment`/`decode_segment`，严格 canonical（re-encode byte-identical），见 `25` |
| Markdown **importer** | **done（round 2）** | `importer::plan_import` + `dispatch_import_markdown`，parity 证明 + 330 文件真实语料浸泡，见 `25` |
| full `anchor-editor-core` intent surface | **done（round 3，见 `26`）** | round 2 +`DeleteText`/`ReplaceText`/`RemoveMark`/`CreateBlock`；round 3 补齐 plan §8.1 全集（indent/outdent/exit-container/transform/insert-code-block/paste-fragment）+ 多块选区编辑 + F21 选择阶梯 |
| 确定性 split/merge **intent-rebase** | **done（round 2，保守规则）** | 单侧 hunks 自动重放，straddle/链式/纯 mark 保持 surface+pin floor，见 `25` |
| `Renormalize` producer + 陈旧坍缩（F26c） | **done（round 2）** | `dispatch_renormalize_children` + replay 端 all-or-nothing CAS 坍缩，见 `25` |
| 公开 **CLI** schema（`cli` 包） | **done（round 3，user-approved 2026-06-10，见 `26`）** | `suites/anchor/cli`（bin `anchor`）：apiVersion 信封、固定退出码、ConflictRecord/resolve/restore 面；契约落 `cli/README.md` |
...
| Open gate | 状态 | Owner |
|---|---|---|
| op-segment round-trippable codec（op-log file format） | **closed（round 2，见 `25`）** | — |
| Markdown importer + import/export parity proof | **closed（round 2，见 `25`）** | — |
| full editor-core intent surface | **closed（round 3，见 `26`）** | — |
| 确定性 split/merge intent-rebase（auto-resolve） | **closed（round 2 保守规则，见 `25`）**；straddle/链式/纯 mark 仍 floor（by design） | core + product |
| `Renormalize` producer + F26c 坍缩 | **closed（round 2，见 `25`）** | — |
| 公开 CLI schema（`cli` 包） | **closed（round 3，user-approved 2026-06-10，见 `26`）** | — |
| **Stage-2 whole-exit** | **human sign-off 非 Apple 部分已签（2026-06-10，`26` §5）**；剩余 gated on CP-2 → CP-1 的 Apple runtime 链 | Apple operator |

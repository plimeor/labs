...
任务：Anchor Stage 1 integration / CP-1 readiness 收口（consolidated cursor + ledger）
日期：2026-06-10
...
本文件整合 core spike 证据（Claude / core owner）与 Apple verifier Stage 1 实测（Codex），并继续作为 CP-1 的 cursor / 进度 ledger。它取代分散的逐次迭代记录。**Consolidated evidence pointers：** `09-apple-verification.md`（Apple 现实核验 + integration state）、`17-apple-binding-evidence.md`（binding）、`18-textkit-adapter-evidence.md`（TextKit/UIKit）、`19-icloud-drive-evidence.md`（iCloud Drive）、`20-cross-target-ci-evidence.md`（cross-target 执行 / CI）。决策契约：`04-contract-baseline.md` / `05-key-decisions.md`（D01–D38）/ `06-fixture-set.md`（F01–F43）/ `10-cp-0-approval.md` / `11-cp-0-final.md`。逐迭代原始 reruns 已 consolidate 进 `09`/`17`/`18`/`19`/`20`；core CP-2 迭代 `72`–`75` 见 §6（Phase 2 将并入 CP-2 readiness doc）。

---

## 1. 结论（CP-1 readiness）

**Core side = complete；CP-1 whole-exit = 未退出 (NOT exited)；本轮无 axis 被 blocked。** Core owner 的确定性 core 已落成正式测试并通过；World A 多目标编译 gate、client 零真理逻辑红线、core 零云符号边界均通过。Codex / Apple verifier 的 binding（组 2）已由用户批准为 B4 产品边界（release-distribution-gated）；TextKit（组 3）机制面通过（产品 runtime 未完成）；iCloud Drive（组 4）已由用户批准为首期 default transport，批准形状为 **approved default transport WITH compromise constraints**。CP-1 整体退出仍 gated on：Apple release distribution、product app 集成、iCloud remote/account/product-context、以及 human sign-off。Stop condition 本轮一项未触发（§4）。

---

## 2. Verified ground truth（Stage 1 Observed）

Repo-local Apple addition 限于 verifier-only 工程；root workspace、lockfile、core、product app target 均未改。命令为可复跑红线。
...
## 3. CP-1 readiness matrix（AUTHORITATIVE，按 axis）

| Axis | Verdict | 依据 |
|---|---|---|
| **core deterministic（组 1+5）** | **go** | 77/0 测试 + 1 ignored、clippy clean、`no_std`+`alloc`+`forbid(unsafe_code)`+零外部依赖、`BTreeMap`/`BTreeSet`（无迭代序不确定）、diff3/order-key/merge/BLAKE3 为 core 内唯一 vendored 实现；`mirror_parity`（structured search == ripgrep(md)、mirror 写失败隔离、body 冲突 git fence）通过。Core side complete。 |
| **multi-target compile（D36）** | **go** | `wasm32-unknown-unknown` + `aarch64-linux-android` 编译 gate 通过；零依赖使 gate by construction 不可被 transitive crate 打破。 |
| **zero-cloud-symbol boundary** | **go** | core 云/文件协调/ubiquity/account token grep = 0 matches, exit 1（live rerun）；core 内唯一 Apple 命中是 `src/lib.rs` 描述性边界注释，无 import/使用。`OpSyncPort` 仅 `SegmentId`/`BlobId` + `&[u8]`，`associated type Error` 让 adapter 用自有 typed error。 |
...
| Stop condition | 触发 |
|---|---|
| 改 root workspace / package / lockfile（或加 placeholder `package.json`） | 否（`members=["core"]`、root 配置未动） |
...
| **CP-1 whole-exit** | **Open — 非 Apple human sign-off 已签（2026-06-10，`22` §5 / `24` §6）；剩余 = Apple operator round** | Needs-Apple-runtime | `22` §5 |

> 此 doc 仍是 CP-1 cursor / ledger。Claude/core 与 human-approval 可达的工作已全部收口（Stage-2 ground floor 关闭、CLI D31 Phase-2 落地、非 Apple whole-exit 签字给出，见 `24`）。下一动作唯余 Apple operator round（signed device + ADP + 真实 iCloud + product app 集成 + distribution identity），或在恢复 hosted CI 时重建 persistent cross-target wiring（须不触 root workspace / lockfile / package-boundary）。

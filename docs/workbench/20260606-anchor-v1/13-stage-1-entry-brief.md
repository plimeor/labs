...
日期：2026-06-07
状态：**workbench artifact** —— 非公开接口契约。本文件是给下一轮 agent 的 CP-1 / Stage 1 spike 启动 briefing：足以直接启动，无需重做 Phase 0 调研。CP-0 已批准（见 `10-cp-0-approval.md` §0）。
...
把 CP-0 冻结的契约方向，通过 Stage 1 五组 spike 推进到「确定性 core + Apple binding + 编辑器事务 + iCloud Drive runtime + mirror parity 作为正式验证通过」的 CP-1 状态。CP-1 退出后才进入 CP-2 的 `anchor-core` 地基实现。**通过 CP-1 前不实现持久应用写入**（plan §11 CP-1；12-stage-1-spike-plan.md §6）。

## 2. Authorized scope

- 在已批准的 Option A `suites/anchor/*` 布局下，**经实现授权后**创建目录 / 工程，执行 `12-stage-1-spike-plan.md` 的五组 spike。
...
- 把任何「必须调整的模型点」回填 `04-contract-baseline.md` / `05-key-decisions.md` / `06-fixture-set.md`（沿用 D01–D38 / F01–F43 编号）。
...
- **硬约束：** 绝不为适配 glob 在 `suites/*/*` 下添加 placeholder `package.json`（会成为 Bun workspace member 并触发 AGENTS package 义务）。详见 `07-project-layout-options.md`、`05-key-decisions.md` D02。
...
- **Binding spike：** 用户已于 2026-06-07 批准 **UniFFI DTO / ordinary dispatch + C ABI bytes fast path** 作为 B4 产品边界。已跑项包括 Rust 三 slice 构建、UniFFI generated Swift full round-trip、C ABI/UniFFI XCFramework、SwiftPM wrapper import、synchronous Swift 6 strict-concurrency release smoke、1/4/16/64MB bytes benchmark。开放项是最终 DTO/error vocabulary、UniFFI async `Sendable` surface、product wrapper/CI 复现。
...
- `11-cp-0-final.md`（CP-0 索引 + 冻结结论 + 禁止扩展）、`10-cp-0-approval.md` §0 / §8（批准摘要 + 勾选）。
- `12-stage-1-spike-plan.md`（五组 spike 的 owner / commands / expected artifact / evidence / failure condition + §6 CP-1 退出条件）。
- `04-contract-baseline.md`（责任基线 + boundary matrix）、`05-key-decisions.md`（D01–D38）、`06-fixture-set.md`（F01–F43）。
- `09-apple-verification.md`（Codex 已 Observed / Not run 边界，避免重证已证项、避免把 Not run 当事实）。
- `./17-apple-binding-evidence.md`、`18-textkit-adapter-evidence.md`、`19-icloud-drive-evidence.md`、`15-core-evidence.md`、`14-core-spike-report.md`（Stage 1 实测证据）。
- `[plan]` `01-apple-native-note-workbench.md`（§8 架构 / §11 阶段 + CP-0 指针）、`[conflict]` `02-conflict-resolution-model.md`（冲突模型权威定义）。
...
完整命令骨架（XCFramework、UniFFI bindgen、Apple target build）见 `12-stage-1-spike-plan.md` §1–§4 与 `09-apple-verification.md` §4.2。
...
- spike 组 4 保留 iCloud runtime evidence matrix：signed container、package UTType、metadata behavior、file coordination、offline conflict materialization、placeholder/account/scale gates。已跑项与开放项见 `19-icloud-drive-evidence.md`。
- **iCloud Drive 作首期默认 transport（B14）已批准，交付 gated on implementation gates：** million-op replay/merge/compaction + steady-state segment budget（N op 不产约 N segment）+ remote placeholder/account-state behavior + product conflict-resolution policy + 四-horizon retention 正确性；delivery no-go-like result triggers batching/transport design reset and user review.
- 任何「必须调整的模型点」回填 04-contract-baseline.md / 05-key-decisions.md / 06-fixture-set.md。
...
- 已批准的 iCloud Drive 默认 transport 出现 delivery no-go-like result（如 N op→约 N synced segment、placeholder/account-state 不可接受、conflict policy 不可呈现）时暂停重评。

...
**Apple 现实标注（贯穿）：** 涉 Apple TextKit / Swift bridging / 双端确定性产物的 fixture 行，其编译 / runtime 机制面为 `Observed`——scratch demo 已证 macOS `NSTextView` runtime probe、`UITextView` 编译面、UTF-16 selection、semantic undo closure（证据见 09-apple-verification.md §6）；Stage 1 已补足 macOS IME marked-text commit、hit-testing insertion index、direct-buffer undo suppression 与 accessibility selected range readback 的机制证据（27-textkit-runtime-floor-report.md；47-textkit-accessibility-range-report.md）。跨 view selection / product accessibility mapping / patch replay / 双端 bit-reproducible 向量集仍为 `Needs Stage 1 spike` 或 open gate。
...
>
> **Apple 机制证据（贯穿 C 区，`Observed`）：** macOS `NSTextView` / iOS `UITextView` adapter 编译面已在 macOS + iOS simulator 编译通过；macOS `NSTextView` runtime probe 已实跑设置 UTF-16 selection、layout manager / text container 可用、`UndoManager` semantic undo closure 可执行（Codex 09-apple-verification.md §2.4）。Stage 1 已观察 macOS IME marked-text commit 到 `EditorIntent.insertText`、hit-testing insertion index、controlled direct-buffer undo suppression，以及 `accessibilitySelectedTextRange()` 回读当前 selected range（27-textkit-runtime-floor-report.md；47-textkit-accessibility-range-report.md）。跨 view selection / product accessibility mapping / VoiceOver/UI runtime / `EditorPatch` replay 到 native view model 仍为 open gate。

...
**CP-2 的 core-owned 不变量已建立并以测试守住；CP-2 whole-exit 仍未退出（gated on CP-1 whole-exit + human sign-off）。** 本轮把 CP-2 的 7 项 core-executable 工作落地 5 项、并把 2 项（确定性 intent-rebase、renormalize 陈旧坍缩）以精确的 remaining-work 记录，因为二者的「正确实现」体量大且若做错会引入确定性回归，宁可记录也不交付半成品。
...
| 4 | 确定性 split/merge **intent-rebase** | **open / documented** | §6 |
| 6 | **renormalize 陈旧坍缩**（F26c） | **open / documented**（reserved 信封变体，无 producer） | §6 |
...
`derive_split_merge_structural` 让原本 dead 的 `ConflictKind::SplitMergeStructural` 变 live：结构 macro 与另一 actor 在同节点的并发普通编辑共存时，surface 冲突并 pin 双方全部 op_id（compaction 永不丢侧）。当前阶段**surface 不 auto-rebase**（auto-rebase 见 §6 item 4）。
...
|---|---|---|---|
| **确定性 split/merge intent-rebase**（item 4） | open / documented | Claude/core | 把并发 body 编辑下的 split/merge macro 基于 pinned diff3 自动 rebase（结果不重复尾文、不静默 LWW 丢侧）。当前安全下限是 §4.3 的「surface + pin」（no silent loss）；auto-resolve 是其上的优化，体量大且需把 split offset 经并发 splice 映射，做错即确定性回归，故记录为 remaining。 |
| **renormalize 陈旧坍缩**（item 6 / F26c） | open / documented | Claude/core | op 信封已 reserve `base_snapshot_revision`，但「base 陈旧的 renormalize 视为 no-op 本地重派生」需 snapshot-pin fixpoint；且当前**无任何 producer 发 `Renormalize` op**（它是 reserved 信封变体，留待未来 compaction/rebalance 子系统）。算法已在 14 §8 / F26 记述，落地留待该子系统。 |
...
- **F26c**：`06-fixture-set.md` 记 renormalize 陈旧坍缩仍 reserved（无 producer）。

...
**CP-2 的 core-owned 不变量已建立并以测试守住；CP-2 whole-exit 仍未退出（gated on CP-1 whole-exit + human sign-off）。** 本轮把 CP-2 的 7 项 core-executable 工作落地 5 项、并把 2 项（确定性 intent-rebase、renormalize 陈旧坍缩）以精确的 remaining-work 记录，因为二者的「正确实现」体量大且若做错会引入确定性回归，宁可记录也不交付半成品。（**后记：两项均已在 round 2 落地，见 `24`** —— intent-rebase 取保守单侧规则、renormalize 取 all-or-nothing CAS 坍缩，surface+pin floor 语义不变。）
...
| 4 | 确定性 split/merge **intent-rebase** | **closed（round 2，见 `24`）** | §6 |
| 6 | **renormalize 陈旧坍缩**（F26c） | **closed（round 2，见 `24`）** | §6 |
...
`derive_split_merge_structural` 让原本 dead 的 `ConflictKind::SplitMergeStructural` 变 live：结构 macro 与另一 actor 在同节点的并发普通编辑共存时，surface 冲突并 pin 双方全部 op_id（compaction 永不丢侧）。当前阶段**surface 不 auto-rebase**（auto-rebase 见 §6 item 4；round 2 已落地保守单侧 rebase，floor 保留给 straddle/链式/纯 mark 情形，见 `24`）。
...
|---|---|---|---|
| **确定性 split/merge intent-rebase**（item 4） | **closed（round 2，见 `24`）** | — | 落地为保守单侧规则：hunks 全在 split 一侧 ⇒ replay 时意图重放（log 不改写、到达序无关）；straddle / 链式 / 纯 mark / macro 形状未通过字节验证 ⇒ 维持 §4.3「surface + pin」floor（no silent loss 语义不变）。 |
| **renormalize 陈旧坍缩**（item 6 / F26c） | **closed（round 2，见 `24`）** | — | producer `dispatch_renormalize_children` + replay 端 per-op location-rev CAS、macro all-or-nothing 坍缩（并发 move 恒胜，部分重铺不致 sibling 乱序）；无 base 的 legacy 信封保持无条件应用。 |
...
| op-shape **formal freeze 签字** | **signed（2026-06-10，用户授权，见 `24` §6）** | — | golden `18582d53…` 自 CP-2 起 pin 住；round 3 的 `supersedes_rev` 消费证明预留字段可被启用而信封零改动。其「授权后续 cloud record schema」的消费端仍属二期 CloudKit（Apple-gated）。 |
| **CP-2 whole-exit** | Open — **human sign-off 非 Apple 部分已签（2026-06-10，`24` §6）**；剩余 gated on CP-1 的 Apple runtime 链 | Apple operator | driver §4：CP-1 whole-exit 先过；CP-1 剩余 = Apple operator round（见 `22` §5）。 |
...
- **F26c**：`06-fixture-set.md` 记 renormalize 陈旧坍缩当时仍 reserved（无 producer）；round 2 已落地 producer + CAS 坍缩（见 `24`）。

...
以下为 contract-baseline.md 与 key-decisions.md 中 Status=Recommended、可作为 CP-0 稳定基线纳入的项；其 Apple 依赖已由 Codex 本机实跑或官方文档直接支持（Observed）。触边界 / 待 Stage 1 spike / 已延后二期的项**不**在本节，集中在第 4、5 节及二期。
...
- **Binding 方向（基线方向 Recommended，机制冻结见第 4、5 节）：** Rust core 保持平台无关，推荐路径 = UniFFI 生成 Swift API（DTO / 结构化 error / 普通 dispatch）+ XCFramework 打包 Rust static libraries + 本地 SwiftPM wrapper 消费，C ABI bytes 作 bulk blob fast-path fallback [key-decisions.md D01；apple-verification §5.3]。**World A 多语言约束：** UniFFI 作多语言绑定生成器（Swift 首期、Kotlin 供 android 后用），web 走 wasm-bindgen / JS；DTO 形状保持绑定生成器无关，同一 core 可被三侧消费而不分叉 DTO。机制可行性 Observed，机制作为产品分发边界的最终冻结待 Stage 1 spike + 用户批准。
...
8. **CloudKit 二期路线 = 已批准（B8，2026-06-07）。** 单附件 cap 已由用户定为 **50MB**（D17，对齐 archived CloudKit `CKAsset` 50MB 上限），原 64MB-vs-50MB 冲突消除——二期 CloudKit / CKSyncEngine **无需**分片 / 降 cap / out-of-band。路线已批准（仍为二期实现）；CloudKit record schema 绝不进 core，op 形状须在任何 CloudKit 记录落地前冻结（key-decisions.md D17；apple-verification §7.3、§8.5）。
...
区分 Observed 与 Not run / Blocked / Unknown，绝不过度声称。完整命令与输出见 apple-verification.md。

**Codex 已 Observed 证明（apple-verification §1、§2.2、§2.4、§2.5、§4、§5、§6、§7）：**
...
- **Host binding 机制：** Rust staticlib + C ABI + SwiftPM executable link/run 成功（实调而非仅编译 header）；UniFFI 0.31.1 经 project-local cargo run 生成并运行 minimal record/bytes Swift binding（`bytes -> Data` 可运行）；one-slice macOS XCFramework 创建成功。
- **TextKit mechanism-only runtime：** `NSTextView` / `UITextView` adapter 编译面 macOS + iOS sim 通过；macOS `NSTextView` runtime 可设 UTF-16 selection、layout manager / text container 可用、`UndoManager` semantic undo closure 可执行。
- **iCloud adapter API 编译面：** `ICloudAdapterProbe` 编译通过；无 entitlement CLI 下 `url(forUbiquityContainerIdentifier:nil)` 返回 nil、`NSMetadataQuery` / `NSFileCoordinator` API 可用。
- **付费 ADP（Individual）team iCloud 能力：** 用户开通付费 Apple Developer Program（`isFreeProvisioningTeam=0`、`teamType = Individual`）后，demo project 经 automatic signing 生成含 iCloud Documents entitlement 的 development provisioning profile + signed device artifact（签名含 `CloudDocuments` + iCloud container / ubiquity ids），并在真机（iOS 26.5、Developer Mode、登录 iCloud）经 `devicectl` 启动返回 **non-nil ubiquity container URL**；simulator 启动不再报 `Security policy issue`（sim ubiquity lookup 仍 nil，因未登录 iCloud）。更早的 free Personal Team 仅能完成普通 app development signing（apple-verification §2.5、§2.6）。
...
- **Anchor target build = Not run。** 仓库无 Anchor Xcode project / scheme，未跑任何 Anchor `xcodebuild` / `swift build` / `cargo build`；所有构建命令只能是 Recommended 命令骨架（apple-verification §1、§2.3）。
- **iCloud entitlement 基础能力 = Blocker 已解除（付费 ADP team）。** free Personal Team 阶段曾 Blocked（`+ Capability` 搜 `iCloud` = `No Matches`、ad-hoc entitlement app 被 SpringBoard 以 `Security policy issue` 拒）；用户开通付费 ADP team 后该门控解除，entitlement provisioning + 真机 ubiquity lookup 已 Observed（见上）。**Anchor 自身 file-package runtime 仍 = Not run**（见下条）（apple-verification §2.5、§2.6、§7.1）。
- **Rust iOS targets 未安装 = Not run 前置。** 仅 `aarch64-apple-darwin` 已装；`aarch64-apple-ios` / `aarch64-apple-ios-sim` 未装，scratch iOS-sim build 因此失败（apple-verification §2.2；key-decisions.md D33）。
- **Anchor DTO binding = Not run。** UniFFI minimal binding 已证机制，未证 Anchor DTO / 结构化 error / async shape / Anchor segment 编码 / BlobId lookup（apple-verification §5）。
- **TextKit IME / 跨 view selection / direct buffer undo 截断 / patch replay / accessibility = Not run。** 仅证 macOS runtime 的 selection / layout / semantic undo closure（apple-verification §6.1）。
- **Anchor iCloud file-package runtime = Not run。** Anchor 自身 file package / package UTType / placeholder download / `NSMetadataQuery` live / file coordination / `NSFileVersion` conflict / signed-out / over-quota 行为均待 Anchor signed app（付费 team 已开通）在真实 account 观测（apple-verification §2.6、§7.4）。
...
以下为已识别、CP-0 后仍开放的现实与设计未决项；它们不阻塞 CP-0 的方向批准，但其收口归 Stage 1 spike 或后续用户决策。
...
- **Core 多目标可编译性（World A）：** 所选 core 依赖（diff3 / blake3 / fractional-index / HLC 等）是否全部能编到 `wasm32` + android，待 CP-1 多目标编译 gate 验证；某依赖不支持 wasm/no_std 时需替换或自实现（Not run，key-decisions.md D36）。
...
- [ ] 批准 / [ ] 暂缓 — **B4 binding 机制作为产品分发边界冻结（Stage 1 证据后）**（第 4 节 #4；key-decisions.md D01）。**按定义须 Stage 1 binding spike 证据后方可冻结，CP-0 阶段保持待定。**
...
- 不写任何 Rust / Swift / TS 代码，不进入 Stage 1 spike 的任何实跑（Stage 1 spike 实跑前须先批准 layout 与目录创建，stage-1-spike-plan.md §0）。

...
- **F23 / F26 双端逐字节一致：** 依赖 pinned、跨平台 bit-reproducible 的 diff3 + order-key 生成器一致性向量集（冲突方案 §13.1 #4 #5）。Codex 已确认：deterministic bytes 应**只在 Rust core 内**生成（非 TextKit/Swift 实现），跨 macOS / iOS by construction 一致；向量集本身为强制 CI gate，落 Stage 1（`Needs Stage 1 spike`，apple-verification.md §1、§8.4）。

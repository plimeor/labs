...
- **id**：稳定标识 `Fnn`，本 packet 内永不复用；后续阶段把它落成测试时引用同一 id。F01–F40 编号沿用 Phase 0 草稿建立的集合（不重排）；F41 为 CP-0 整合期新增的边界 fixture（local-only 误放入 iCloud，D21a）。
- **name**：人类可读名。
...
- **F23(c)/F25/F34 watermark 与 compaction 交互：** 依赖主计划 §11「op-log compaction、GC 保留窗口与 manifest / cursor 协调」与冲突方案 §10 / §13.1 #6。

...
- **iCloud Drive adapter spike：** Stage 1 状态 = **compromise**。已跑项包括 signed iPhone/macOS runtime、ubiquity container、vault package UTType、package-level metadata discovery、`NSFileCoordinator` read/write、package-internal direct enumeration、1024-file subset、online convergence、offline `NSFileVersion` conflict materialization、core 云符号审计。开放项是 product conflict-resolution policy、remote placeholder download、signed-out / over-quota、segment-file-count scale（10K/50K/100K）、local-only 路径判定边界（D21/D21a）、repo-local product app target entitlement。

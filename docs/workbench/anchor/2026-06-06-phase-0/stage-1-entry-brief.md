...
- **前置：** `rustup target add aarch64-apple-ios aarch64-apple-ios-sim`（host `aarch64-apple-darwin` 已装；iOS targets 未装为硬前置，D33）；Apple 命令用 `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer` 前缀 + `-derivedDataPath` 避免 DerivedData 落 repo。
- **Binding spike：** Rust 三 slice 构建 + UniFFI 对 Anchor DTO（`open_fixture_vault`、`dispatch(EditorIntent) -> TransactionResult`、`read_segment(SegmentId) -> bytes`）+ structured errors + async-Sendable（Swift 6 strict concurrency）+ 1/4/16/64MB bytes transfer 成本 + XCFramework 三 slice + SwiftPM wrapper import；据 pass criteria 决定 UniFFI primary vs UniFFI DTO + C ABI fast path。
...
- **iCloud Drive adapter spike：** Anchor target iCloud-capability 签名（付费 ADP team 已开通）、ubiquity container、vault file package `com.apple.package` UTType、coordinated write 可见性、`NSMetadataQuery` live、placeholder download、`NSFileCoordinator` 不阻塞 UI、manifest 并发写（`NSFileVersion`，默认 per-device immutable cursor）、signed-out / over-quota、core 云符号审计；**segment-file-count scale（1K/10K/50K/100K）+ 真机 go/compromise/no-go**；local-only 路径判定边界（D21/D21a）。
...
- spike 组 2、3 有可重复命令 / Xcode scheme + 报告，列出 binding round-trip / bytes benchmark 与 single-block / block / embedded / 跨 block 选择的通过 / 失败证据。
- spike 组 4 在付费 team + 授权 profile 前置满足后给出 iCloud runtime 证据，或明确标 Blocked / Not run。
- **iCloud Drive 作首期默认 transport（B14）gated on scale gate：** million-op replay/merge/compaction + steady-state segment budget（N op 不产约 N segment）+ iCloud Drive 真机 go/compromise + 四-horizon retention 正确性；no-go 转 CloudKit / 中立 object-store。
...
- binding spike 出现 Anchor DTO 不支持 / Swift 6 strict concurrency 失败 / structured error 退化 strings / bytes transfer 过慢或内存尖峰过高 → 重评含转 C ABI primary（D01）。
- Kleppmann 祖先检查实现引入 move 历史保留（滑向完整收敛 move）；time-travel 范围升级为需保留 move 历史的结构重演。
- Stage 1 实测 shared mutable manifest conflict noisy（CP-1 前重设 manifest write 方案，不得回退为允许截断 open-conflict op）。

...
>
> 引用约定：`[plan §X]` 指 `docs/plans/2026-06-06-anchor-apple-native-note-workbench.md`；`[conflict §X]` 指 `docs/plans/2026-06-06-anchor-conflict-resolution-model.md`；Phase 0 Codex 验证证据指同目录 `apple-verification.md`；Stage 1 实测证据指 `docs/workbench/anchor/2026-06-07-stage-1/`。同目录姐妹文件用相对名引用（如 `key-decisions.md`、`fixture-set.md`、`project-layout-options.md`）。
>
...
五条支柱**可进入 CP-0**（Observed，apple-verification §1）：Apple-native 首期路线、Rust `anchor-core` 真理层、Apple 客户端进程内 binding、TextKit mechanism-only、iCloud Drive adapter 只给 core 喂 `SegmentId` / `BlobId` + 字节。在此之上，Stage 1 Apple evidence 给出三条当前基线：binding 机制采用 **UniFFI DTO / ordinary dispatch + C ABI bytes fast path**；Apple 工程位置 = **用户已批准 Option A `suites/anchor/*`（2026-06-07）**，Option C 仅作实现期 Bun-glob / Xcode 嵌套成本过高时的退路；iCloud Drive runtime 为 **compromise**，可观察 signed container / file package / coordinator / conflict materialization，但 package-internal segment discovery 不依赖 `NSMetadataQuery`，default transport 仍受 scale、placeholder、account-state 与 conflict-resolution policy 门控。

**显式限定：实现仍需单独授权。** 本结论是契约方向的对齐，不是产品 app 实现许可；plan §10 把「授权后的第一个实现单元」定义为阶段0，§13 保留若干必须用户决策的岔口（package/workspace/Apple project 边界变化、binding 成本不可接受、同步路线产品级选择等）[plan §13]。Stage 1 已在 `suites/anchor/core` 与 `suites/anchor/apple` 落成最小验证工程，并在 repo 外 Xcode probes 验证 signing/runtime；这些 spike 不是产品 app shell，不授权持久应用写入，也不把 iCloud Drive default transport 升级为已批准。
...
- **iCloud Drive vault file package 必须声明符合 `com.apple.package` 的 exported document type / UTType（Observed-supported，apple-verification §7.1；icloud-drive-report.md）。** Vault/package discovery 使用 package-level metadata；package-internal segment discovery 使用 file-coordinated direct enumeration，不能依赖 `NSMetadataQuery` 枚举 package internals。
...
- **iCloud Documents / ubiquity runtime = compromise（apple-verification §2.6、§7；icloud-drive-report.md）。** 用户已开通付费 Apple Developer Program（Individual）team（`isFreeProvisioningTeam=0`）；signed iPhone and macOS probes returned non-nil ubiquity containers, validated package type id, coordinated read/write, 1024-file write subset, package-level metadata discovery, package-internal direct enumeration, and offline `NSFileVersion` conflict materialization. Placeholder remote download, signed-out / over-quota, local-only path edge cases, and 10K/50K/100K file-count scale remain open gates.
...
**TextKit runtime status = partial observed（textkit-adapter-report.md）。** Stage 1 covers macOS UTF-16 emoji/ZWJ/combining/CRLF offset smoke, layout, selection, semantic undo through adapter-owned `UndoManager`, and iOS simulator compile. Real app view hierarchy, input event capture, IME marked text, accessibility range, direct buffer undo suppression in responder chain, and full patch replay remain runtime gates.

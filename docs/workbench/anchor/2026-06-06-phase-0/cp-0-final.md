...
以下状态来自 2026-06-07 Stage 1 workbench。已验证项进入当前决策基线；开放 gate 保留为 CP-1 / default transport / 产品签署前置。

- **Apple binding 方向（A2 / B4 / D01）：** Stage 1 evidence supports **UniFFI DTO / ordinary dispatch + C ABI bytes fast path**. Final product freeze still requires DTO/error vocabulary and wrapper packaging signoff.
- **iCloud Drive 作首期默认 transport（B14）：** current status = **compromise**. Signed runtime, package-level discovery, file coordination, direct package-internal enumeration, and offline `NSFileVersion` conflict materialization are observed. Default transport approval remains gated on segment-file-count scale（1K/10K/50K/100K）, million-op replay/merge/compaction, steady-state segment budget, placeholder/account-state behavior, and product conflict-resolution policy.
- **iCloud Anchor file-package runtime（D35 / D14）：** `.anchorvault` package UTType, package-level `NSMetadataQuery`, `NSFileCoordinator` read/write, 1024-file subset, online convergence, and offline conflict materialization are observed. Package-internal segment discovery uses direct enumeration; `NSMetadataQuery` is not approved for package-internal `.seg` discovery.

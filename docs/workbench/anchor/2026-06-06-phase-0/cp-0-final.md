...
- **iCloud Drive 作首期默认 transport（B14）：** current status = **compromise**, not approved default transport. Signed runtime, package-level discovery, file coordination, package-internal direct enumeration through macOS 100K files, online convergence, and offline `NSFileVersion` conflict materialization are observed. Default transport approval remains gated on remote placeholder behavior, signed-out / over-quota states, iOS large-scale delivery, steady-state segment budget, and product conflict-resolution policy.
...
当前入口是 `../2026-06-07-stage-1/stage-1-integration-report.md`。下一步只处理剩余 open gates：binding product-freeze signoff、iCloud default transport gate、TextKit product-runtime gate、Bun glob check、cross-target execution CI wiring。通过 CP-1 前不实现持久应用写入。

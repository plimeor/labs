...
- **Status**：Recommended（Stage 1 binding + synchronous release-surface evidence passed；最终 DTO/error vocabulary、product wrapper surface、CI/fresh-machine 复现与产品分发签署仍需用户确认）
...
- **Risk**：UniFFI async `Sendable` surface 仍有已知 rough edge；高度递归/泛型 DTO 在 FFI 边界表达力存疑；bulk blob hot path 用 UniFFI `Data` 会付出更高复制与内存成本；最终生产 DTO/error vocabulary、product wrapper surface 与 CI/fresh-machine 复现仍需签署。
- **Stop condition**：若最终 Anchor DTO shape 不被 UniFFI 干净支持、final generated Swift surface 无法过 Swift 6 strict concurrency、structured error 语义无法保持、bulk bytes fast path 无法保留、或 async boundary 要求 Anchor 不可接受的 Rust runtime 假设 → 暂停并重评（含 C ABI primary）。绑定作为产品分发边界冻结须用户签署。
